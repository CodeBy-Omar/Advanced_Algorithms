...
// Definition of the ExpressionTree class
...
public:
    ...
    void display() {
        ...
        // so if the depth is 5 and width is 3. it works inside out, the string function create string given 2 parameters.
       // first parameter is how many time it is repeated, and sec one is the actual string to be repeated
...
private:
    ...
    unique_ptr<Node> buildTree(const string& expr) {
        ...
        string cleanExpr = sanitize(expr); // Sanitize the expression by removing outer parentheses
        ...
        // Loop through the expression to find operators outside of any parenthesis
        for (int i = 0; i < len; i++) {
    ...
    void fillDisplay(vector<string>& lines, Node* node, int row, int col, int depth) {
        ...
        if (node->left) fillDisplay(lines, node->left.get(), row + 1, col - branchLength, depth); // Recursively display left child
        if (node->right) fillDisplay(lines, node->right.get(), row + 1, col + branchLength, depth); // Recursively display right child
    }
};

int main() {
    ExpressionTree exprTree; // Create an instance of ExpressionTree
    string expression = "((7+6)/((7+8+9)*9))"; // Define the expression to be parsed and evaluated
